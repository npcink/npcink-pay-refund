...
//存储数据
  ...
  const getData = async () => {
    ...
    try {
      const response = await axios.post(`${dataLocal.route}pf/v1/get_option`, datas, {
        headers: {
  ...
  // 使用解构语法来获取响应数据对象中的属性，并为每个属性提供默认值以避免未定义的属性
      const {
        npc_refund_user = [],
        zfb: { npc_zfb_appid: zfbAppid = '', npc_zfb_private_key: zfbPrivateKey = '', npc_zfb_public_key: zfbPublicKey = '' } = {},
        wx: { npc_wx_mch_id: wxNpcRefundMchId = '', npc_wx_cert_api: wxCertApi = '', npc_wx_cert_key: wxCertKey = '' } = {},
      } = responseData;
  ...
  return {datas,config, getData, saveData };
});
