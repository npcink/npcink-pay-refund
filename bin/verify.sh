...
if ! grep -Fx "${PLUGIN_SLUG}/vendor/autoload.php" <<< "$ZIP_LIST" >/dev/null; then
	echo "Release zip is missing vendor/autoload.php." >&2
	exit 1
fi
if grep -E "^${PLUGIN_SLUG}/(vite/|bin/|build/|admin/sdk/guzz/|admin/sdk/zfb/)" <<< "$ZIP_LIST" >/dev/null; then
...
if command -v wp >/dev/null 2>&1 && [ -d "$WP_PATH" ]; then
	wp plugin check "$PLUGIN_SLUG" \
		...
		--exclude-files=.gitignore,.distignore >/tmp/magick-refund-pcp-verify.out

	php <<'PHP'
<?php
$lines = file('/tmp/magick-refund-pcp-verify.out', FILE_IGNORE_NEW_LINES);
$rows = array();
